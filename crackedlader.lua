...
local Window = OrionLib:MakeWindow({Name = "LadderBreaker - dolce milk obby CRACKED BY BELB", HidePremium = false, IntroEnabled = true, IntroText = "Loading..", SaveConfig = true, ConfigFolder = "laderAHHFolder"})
--locals
local PlayerLog = game.Players.LocalPlayer

OrionLib:MakeNotification({
	Name = "LadderBreakerCRACK loaded",
	Content = "created by m1kpee,CRACKED BY belb_18683",
	Image = "rbxassetid://4483345998",
	Time = 6
})

OrionLib:MakeNotification({
	Name = "You are logged in as "..PlayerLog.Name.."",
...
function brkldrfull()
	while _G.breakfullLadder == true do
		Game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(88, 141, -237)
		wait(_G.brkspeed)
		Game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(90, 140, -234)
		wait(_G.brkspeed)
...
function m1kpeehasjoined()
	OrionLib:MakeNotification({
		Name = "creater of CRACK has joined",
...
game.Players.PlayerAdded:Connect(function(plr)
	if plr.Name == "belb1337" then
		print("creator of CRACK has joined")
		m1kpeehasjoined()
	end
end)
...
MainTab:AddToggle({
	Name = "Break ladder (easier to pass)",
	Default = false,
...
MainTab:AddToggle({
	Name = "Break more (impossible??)",
	Default = false,
	Callback = function(Value)
		_G.breakfullLadder = Value
        brkldrfull()
	end    
})
...
MainTab:AddParagraph("WARNING","new gamepass: the main admin, THEY CAN KICK AND BAN PEOPLE!!")
MainTab:AddParagraph("invisible breaking (need systembroken)","just attach other player (u can do: headsit, backpack, bang, stand, doggy, drag)")
MainTab:AddParagraph("Speed","lower num = speeder breaking, higher num = slower")
--teleport
...
--anti admin
local AAdminTab = Window:MakeTab({
	Name = "Anti-admin",
        Image = "rbxassetid://4483345998",
	PremiumOnly = false
})
...
Ctab:AddParagraph("v3 here!","better ladder breaking script, rewrited player tab, rewrited anti-admin tab, added key (i need money, sorry)")
Ctab:AddParagraph("v3.1 here!"," renamed color tab, cleared changelog tab, copy link button new key system with linkversite (more money xd)")
--creator tab
